...
CREATE TABLE `system_message` (
  ...
  `content` TEXT NOT NULL COMMENT '消息内容',
  `type` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '消息类型：1-欢迎消息，2-反馈回复，3-系统通知',
  ...
  KEY `idx_status` (`status`),
  KEY `idx_create_time` (`create_time`),
  CONSTRAINT `fk_system_message_user` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`) ON DELETE CASCADE
